    ...
    def __init__(self, is_training,vocab_size,label_size):
        def lstm_cell():
            lstm=rnn.BasicLSTMCell(
                config.hidden_size, forget_bias=0.0, state_is_tuple=True, reuse=tf.get_variable_scope().reuse)
            if is_training:lstm=rnn.DropoutWrapper(lstm,output_keep_prob=0.5)
            return lstm
        with tf.name_scope('input'):
            self.input_data=tf.placeholder(tf.int64,[None,config.seq_length],name='input_x')
        ...
        with tf.variable_scope('embeddingLayer'):
            ...
            inputs = tf.split(embedded, config.seq_length, 1)
            inputs = [tf.squeeze(input_, [1]) for input_ in inputs]



        with tf.variable_scope('rnnLayer'):
            self.cell = rnn.MultiRNNCell([lstm_cell() for _ in range(config.num_layers)])
